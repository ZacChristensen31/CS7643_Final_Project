...
#                 r"C:\Users\jglicksm\git\CS7643_Final_Project\TL-ERC\WAV2VEC")
...
class CustomCombined(nn.Module):

    def __init__(self, config):
        super(CustomCombined, self).__init__()

        # Default settings for device and config
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.config = config

        # Make sure we have out models set up -
        self.text_model = bc_RNN(config)
        self.audio_model = OpenSmile(config)
        # self.visual_model = OpenSmile(config)

        # Here we get quite simple - multiple layers, fully connected
        # that take in the combined logits from the models.

        # TODO: Only use the audio and text models (visual and customization
        # can be added in later if needed, hopefully dynamically)
        self.network = nn.Sequential(
            nn.Linear(2 * config.num_classes, 256),
            nn.Sigmoid(),
            nn.Linear(256, config.num_classes),
        )

    # We know the forward function is just taking in 2 * num_classes logits, and
    # don't really have many more details than that. Assume that the input to this
    # already has passed through the models and is already combined.
