...
class IEMOCAP:
    ...
    def load_iemocap_data(self):
        ...
        self.trainVid, self.valVid = train_test_split(
            list(self.trainVid_raw), test_size=.2, random_state=1227)
...
def pad_sentences(conversations,
    ...
    def pad_tokens(tokens, max_sentence_length=max_sentence_length):
        ...
        tokens = list(tokens) + [eos_token] + [pad_token] * n_pad
        return tokens
...
if __name__ == '__main__':
    ...
    for split_type in ['train', 'valid', 'test']:
        ...
        conversation_length = [min(len(conv), max_conv_len) for conv in conv_sentences]
